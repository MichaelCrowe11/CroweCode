...
// Rate limiting state (in-memory; for multi-instance use Redis based adapter)
...
const io = new Server(httpServer, {
  cors: {
    origin: (process.env.CORS_ORIGIN ? process.env.CORS_ORIGIN.split(',').map((o: string) => o.trim()) : '*') as any // simplified; allow string[] | '*'
  }
});
...
function attachHandlers(nsp: Namespace, label: string) {
  nsp.on('connection', (socket: Socket) => {
    ...
    socket.on('message', (payload: unknown) => {
      ...
      if (typeof payload === 'object' && payload && (payload as any).room) {
        const { room, data } = payload as any;
...
async function pushMetrics() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error('Failed to push metrics:', (err as Error).message);
  }
}
...
export {}; // Ensure treated as a module